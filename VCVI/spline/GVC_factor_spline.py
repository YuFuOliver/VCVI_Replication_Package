...
class GVC_factor_spline:
    ...
    def __init__(self, k, p, isYJ, optimizer, sampling, 
        ...
        self.num_group = k.size
        self.k = k

        self.dim1 = k[0]+k[1]+k[2] # the dim of betas
        ...
        self.l32 = torch.zeros(self.k[2]-1, requires_grad = True)
        self.l13 = torch.zeros(self.k[0]-2, requires_grad = True)
        self.l23 = torch.zeros(self.k[1]-2, requires_grad = True)
        self.l33 = torch.zeros(self.k[2]-2, requires_grad = True)
        ...
        vari_para = [self.b, self.s, self.l12, self.l22, self.l32, self.l13, self.l23, self.l33, self.Ct, self.zetat]
    ...
    def sample(self) -> Tuple[TensorWithGrad, torch.Tensor]:
        ...
        theta, A_inv = GVC_factor_spline.rep_trick(self.b,self.s,L,self.Ct,self.zetat,self.etat,self.k,self.p,self.identities,self.isYJ)
    ...
    def train(self, num_iter=10000):
        ...
        l32_store = torch.zeros(1000, self.k[2]-1)
        l13_store = torch.zeros(1000, self.k[0]-2)
        l23_store = torch.zeros(1000, self.k[1]-2)
        l33_store = torch.zeros(1000, self.k[2]-2)
        ...
        for iter in range(num_iter):
            ...
            if iter >= num_iter - 1000:
                with torch.no_grad():
                    ...
                    l32_store[iter - (num_iter - 1000)] = self.l32.clone().detach()
                    l13_store[iter - (num_iter - 1000)] = self.l13.clone().detach()
                    l23_store[iter - (num_iter - 1000)] = self.l23.clone().detach()
                    l33_store[iter - (num_iter - 1000)] = self.l33.clone().detach()
        ...
        with torch.no_grad():
            ...
            avg_l32,_ = torch.median(l32_store, dim=0)
            avg_l13,_ = torch.median(l13_store, dim=0)
            avg_l23,_ = torch.median(l23_store, dim=0)
            avg_l33,_ = torch.median(l33_store, dim=0)
        ...
        if self.sampling:
            ...
            for i in range(sample_size):
                theta,_ = GVC_factor_spline.rep_trick(avg_b,avg_s,L,avg_Ct,avg_zetat,avg_etat,self.k,self.p,self.identities,self.isYJ)
